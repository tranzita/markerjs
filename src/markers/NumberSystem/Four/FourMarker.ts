...
import { FourMarkerState } from './FourMarkerState';

export class FourMarker extends RectangularMarkerBase {
    public static createMarker = (): FourMarker => {
    ...
    constructor() {
        super();
        this.markerTypeName = 'FourMarker';
    }
    protected readonly MIN_SIZE = 50;

    private readonly DEFAULT_TEXT = '4';
    private text: string = this.DEFAULT_TEXT;
    private textElement: SVGTextElement;
    private inDoubleTap = false;
    private editor: HTMLDivElement;
    private editorTextArea: HTMLTextAreaElement;
    public getState(): FourMarkerState {
        const state: FourMarkerState = Object.assign(
            { text: this.text }, super.getState()); 
        return state;
    }
    public restoreState(state: FourMarkerState) {
    ...
    protected setup() {
        ...
        SvgHelper.setAttributes(this.visual, [["class", "four-marker"]]);
